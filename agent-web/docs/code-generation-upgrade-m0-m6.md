...
- 建立 `BusinessRequirement -> Requirement IR` 的确定性规范化器。
- 将模型输出限制为 Schema 校验后的 IR。
- 从 IR 独立派生基础验收断言。
- 对重大歧义阻断，对默认型歧义记录明确决议。

验收：IR 可追溯、可版本化；相同确认需求产生等价 IR；重大歧义不会进入代码生成。

### M2：确定性生成

- 用代码生成器负责固定 import、页面骨架、共享 Shell、提交载荷和测试装配。
- 模型仅负责受限布局、表达式和无法模板化的 glue code。
- 生成代码只能表达 IR 中存在的行为。

验收：30 个任务技术门禁全部通过；关键业务断言全部通过。
...
- 检索库只收录技术门禁、业务验收和人工审核均通过的样例。
- 先使用版本/业务能力元数据过滤，再做全文或 BM25 排序。
- 工具先返回 key、摘要、版本、hash 和分数，模型按 key 读取冻结原文。
- 记录 query、过滤条件、候选集和最终读取项。

验收：在同一模型、Prompt 和任务集上做成对 A/B；证明检索改善至少一种预先声明的失败类别，且没有降低关键断言通过率。

### M5：本地向量增强

- 仅当 M4 证实存在稳定的语义漏召回时启用。
- 在 `Retriever` 接口后增加本地 embedding 与向量召回，与全文结果融合排序。
- embedding 模型、切块器和索引版本全部进入快照。

验收：相较 M4 的全文基线有可重复的边际收益；不新增外部数据接收方。

### M6：影子发布

- 每次 generation 记录 strategy version。
- 新旧链路对同类任务影子比较，先小范围启用，再设为默认。
- 保留快速回退旧策略的能力。

验收：满足技术、业务、延迟和成本门限；发布期间无关键业务断言回归。
