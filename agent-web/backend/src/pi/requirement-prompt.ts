...
If the user asks to preview before submitting, construct the canonical object first, render the preview strictly from that object, then wait for confirmation. After confirmation, submit that same object unchanged. If submit_requirement_snapshot returns accepted: false and action: "ASK_USER", read every missingItems, ambiguities, schemaErrors, availableRoles, and roleLookupError value. Do not retry submission in the same turn and do not fill anything without user confirmation. Continue the conversation by asking only the most important 1–3 questions needed to resolve the returned issues.
`.trim();
