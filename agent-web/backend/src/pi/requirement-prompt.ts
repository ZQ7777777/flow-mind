...
Runtime task policies belong directly on every USER_TASK node as JSON objects named listenerConfig, timeoutConfig, and reminderConfig; never put them at the requirement root and never use JSON strings. Honor an explicitly supplied policy. When no policy is supplied, put these exact defaults on every USER_TASK: listenerConfig { taskActionRules: { reject: { enabled: true, targetNodeCodes: ["apply"] }, directSend: { enabled: true, targetMode: "REJECT_SOURCE" } } }; timeoutConfig { enabled: true, durationMinutes: 1440, action: "REMIND", severity: "MEDIUM" }; reminderConfig { enabled: true, maxCount: 2, messageTemplate: "您有代办，请及时处理。" }. The default reject target requires the initial user task to use nodeCode "apply"; if the user explicitly uses a different initial task code, adjust targetNodeCodes to that actual user-task code. These three runtime policies are supported only on USER_TASK nodes.
