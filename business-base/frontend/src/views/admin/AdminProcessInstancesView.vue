...
const query = reactive({ pageNo: 1, pageSize: 10, processCode: "", instanceStatus: "", instanceTitle: "", starterUserId: "" });
...
function resetQuery(): void {
  Object.assign(query, { pageNo: 1, processCode: "", instanceStatus: "", instanceTitle: "", starterUserId: "" });
  void load();
}
...
<template>
  <section class="page-surface admin-page">
    ...
    <form class="filters" @submit.prevent="query.pageNo = 1; load()">
      <label>流程编码<input v-model="query.processCode" /></label><label>实例标题<input v-model="query.instanceTitle" /></label><label>发起人 ID<input v-model="query.starterUserId" /></label>
    ...
    <div v-if="detailOpen" class="modal-backdrop detail-backdrop" data-test="instance-detail-backdrop" @click.self="closeDetail">
      <section class="detail-modal" role="dialog" aria-modal="true" aria-labelledby="instance-detail-title">
        <header class="detail-header">
          <div>
            <p class="eyebrow">{{ selected ? `${selected.processCode} · ${selected.instanceStatus}` : '流程实例' }}</p>
            <h3 id="instance-detail-title">{{ selected?.instanceTitle || '实例详情' }}</h3>
            <small v-if="selected">{{ selected.instanceId }}</small>
          </div>
          <div class="detail-header-actions">
            <template v-if="selected">
              <button :disabled="!canTerminate" @click="terminateSelected">终止流程</button>
              <button class="danger" @click="deleteSelected">删除实例</button>
            </template>
            <button @click="closeDetail">关闭</button>
          </div>
        </header>
        <div v-if="detailLoading" class="detail-loading" role="status">正在加载实例详情...</div>
        <div v-else-if="selected" class="detail-body">
          ...
          <section><h4>流程变量</h4><div class="table-wrap"><table><thead><tr><th>变量</th><th>值</th></tr></thead><tbody><tr v-if="variableRows.length === 0"><td colspan="2" class="empty">暂无变量</td></tr><tr v-for="item in variableRows" :key="item[0]"><td>{{ item[0] }}</td><td>{{ formatUnknown(item[1]) }}</td></tr></tbody></table></div></section>
          <section><h4>活动任务</h4><div class="table-wrap"><table><thead><tr><th>节点</th><th>办理人</th><th>候选人</th><th>状态</th><th>创建时间</th></tr></thead><tbody><tr v-if="activeTasks.length === 0"><td colspan="5" class="empty">暂无活动任务</td></tr><tr v-for="task in activeTasks" :key="String(task.taskId)"><td>{{ task.nodeName || task.nodeCode }}</td><td>{{ task.assigneeUserName || task.assigneeUserId || '-' }}</td><td>{{ formatUnknown(task.candidateUserIds) }}</td><td>{{ task.taskStatus }}</td><td>{{ formatDateTime(task.createdAt as string) }}</td></tr></tbody></table></div></section>
          <ProcessGraph v-if="definition" :nodes="definition.nodes" :edges="definition.edges" :current-node-codes="selected.currentNodeCodes ?? []" />
        </div>
      </section>
    </div>

    <div v-if="traceOpen" class="modal-backdrop trace-backdrop" @click.self="traceOpen = false"><section class="trace-modal" role="dialog"><header><h3>{{ traceTitle[traceType] }}</h3><button @click="traceOpen = false">关闭</button></header><div class="table-wrap"><table><thead><tr><th v-for="column in currentColumns" :key="column[0]">{{ column[1] }}</th></tr></thead><tbody><tr v-if="!traceLoading && traceRows.length === 0"><td :colspan="currentColumns.length" class="empty">暂无记录</td></tr><tr v-for="(row,index) in traceRows" :key="index"><td v-for="column in currentColumns" :key="column[0]">{{ cell(row,column[0]) }}</td></tr></tbody></table></div><footer class="pagination"><span>共 {{ traceTotal }} 条</span><button :disabled="tracePage <= 1" @click="tracePage--; loadTrace()">上一页</button><span>第 {{ tracePage }} 页</span><button :disabled="traceRows.length < 20" @click="tracePage++; loadTrace()">下一页</button></footer></section></div>
  </section>
</template>

<style scoped>
.admin-page{display:grid;gap:16px}.page-header,.detail-header,.trace-modal>header,.pagination{display:flex;align-items:center;justify-content:space-between;gap:12px}h2,h3,h4,p{margin:0}.eyebrow{margin:0 0 4px;color:#0f766e;font-size:12px;font-weight:800}button{min-height:34px;border:1px solid #d8dee8;border-radius:6px;padding:6px 12px;background:#fff;color:#17202a;cursor:pointer;font:inherit}button:hover:not(:disabled){border-color:#2563eb;color:#2563eb}button:disabled{opacity:.55;cursor:not-allowed}.primary{background:#0f766e;color:#fff;border-color:#0f766e}.primary:hover:not(:disabled){background:#115e59;color:#fff;border-color:#115e59}.danger,.detail-header-actions .danger{color:#b91c1c}
.filters{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;align-items:end}.filters label{display:grid;gap:6px;color:#5d6978;font-size:13px}.filter-actions,.trace-actions,.detail-header-actions{display:flex;gap:8px;align-items:end;flex-wrap:wrap}input,select{width:100%;min-height:34px;border:1px solid #d8dee8;border-radius:6px;padding:7px 9px;background:#fff;color:#17202a;font:inherit}.table-wrap{overflow:auto;border:1px solid #d8dee8;border-radius:8px;background:#fff}table{width:100%;min-width:760px;border-collapse:collapse;table-layout:fixed}.admin-page>.table-wrap th:last-child,.admin-page>.table-wrap td:last-child{width:110px}th,td{overflow-wrap:anywhere;border-bottom:1px solid #d8dee8;padding:9px 10px;text-align:left;vertical-align:middle}th{position:sticky;top:0;z-index:1;height:40px;background:#f8fafc;color:#344054;font-size:12px;font-weight:700}td{color:#17202a;font-size:13px}td small,.detail-header small{display:block;margin-top:4px;color:#5d6978;font-size:12px}tbody tr:hover,tr.selected{background:#eef6ff}.empty{padding:20px;background:#f8fafc;color:#5d6978;text-align:center}.pagination{justify-content:flex-end;color:#5d6978;font-size:13px}
.modal-backdrop{position:fixed;inset:0;z-index:20;display:grid;place-items:center;padding:22px;background:rgba(15,23,42,.55)}.detail-modal{display:grid;grid-template-rows:auto minmax(0,1fr);width:min(1180px,96vw);max-height:94vh;border-radius:9px;background:#fff;box-shadow:0 24px 70px rgba(0,0,0,.3);overflow:hidden}.detail-header{padding:14px 18px;border-bottom:1px solid #e2e8f0}.detail-body{display:grid;gap:18px;overflow:auto;padding:16px 18px}.detail-body>section{display:grid;gap:10px}.detail-loading{min-height:220px;padding:72px 18px;color:#5d6978;text-align:center}.summary{display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:12px;margin:0;padding:12px;background:#f8fafc}.summary dt{color:#64748b;font-size:12px}.summary dd{margin:4px 0 0}.trace-backdrop{z-index:30}.trace-modal{display:grid;gap:12px;width:min(1000px,94vw);max-height:88vh;overflow:auto;padding:18px;border-radius:9px;background:#fff;box-shadow:0 24px 70px rgba(0,0,0,.3)}.trace-modal>header{position:sticky;top:-18px;z-index:2;padding:4px 0 12px;background:#fff;border-bottom:1px solid #e2e8f0}
@media(max-width:900px){.filters{grid-template-columns:1fr}.summary{grid-template-columns:1fr 1fr}.detail-modal{width:98vw}.detail-header{align-items:stretch;flex-direction:column}}
</style>
