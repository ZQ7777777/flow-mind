...
缓存键使用 `definitionId + version`。只有已发布、已激活且通过完整校验的定义才能进入缓存；草稿、未激活、校验失败、加载异常或半成品不得缓存。

```mermaid
flowchart TD
    R["运行时请求 definitionId + version"] --> C{"缓存命中"}
    C -->|是| S["返回不可变快照"]
    C -->|否| L["读取 A/C 完整定义数据"]
    L --> V["执行发布语义校验"]
    V -->|失败| E["拒绝加载，不写缓存"]
    ...
    A -->|否| E
    A -->|是| G["解析不可变运行时图"]
    G --> P["按 definitionId + version 缓存并返回"]
```

A 保存、发布、激活、停用、归档或删除定义，或 C 保存表单/附件配置后，必须在数据库事务成功提交后按 `definitionId` 失效全部版本缓存。事务回滚不得误删当前有效快照。第一阶段使用 Java 8 线程安全内存结构，不引入 Redis 等分布式缓存。
