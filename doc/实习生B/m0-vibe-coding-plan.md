...
所有 DTO 保持 Java 8 JavaBean 契约：允许手写无参构造与 getter/setter，也允许使用 Lombok `@Data`；无论采用哪种方式，都必须可无参构造并具备标准访问器。禁止使用 `record` 和 Java 9+ API。
...
| 依赖方 | 必须提供 | B 的处理 |
|---|---|---|
| A | 实例、任务、历史模型字段确认，以及持久化字段、状态取值和乐观锁语义 | B 在 A 合并前不自行猜测持久化映射；A 合并后按确认字段校正 DTO。 |
| C | `api.enums` 中的状态/动作枚举、`WorkflowEvent`、SPI、Starter Service 接口及幂等测试基线 | B 在 C 合并前不创建替代枚举、事件或 SPI；仅准备不依赖共享类型的 DTO 与请求夹具，待 C 合并后完成正式类型替换、编译与联合测试。 |
...
1. **B0.1：请求基类**
   ...
   提示词：  
   “在 `platform-core` 创建 Java 8 DTO 基类。所有修改请求继承 `OperationRequest`，任务级请求继承 `TaskOperationRequest`。只定义字段和 JavaBean 访问契约，可手写无参构造与 getter/setter，也可使用 Lombok `@Data`；不新增枚举、SPI、持久化或业务逻辑。”
