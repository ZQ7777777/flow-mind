...
4. 基于已确认需求生成“入金申请”的 Spring Boot 业务发起适配代码和 Vue3 录入页面。
...
| 入金申请代码生成 | 推理和工具调用 | 编排、暂存和约束 | 提供目标契约 |
...
- 提交请求和响应 DTO。
- 表单校验、流程变量和银行回单附件映射。
- `POST /api/entry-application/submit`。
...
响应至少返回实例 ID、实例状态和 `createdTasks` 中的下一节点任务摘要。页面只展示提交成功和下一处理节点提示，不实现审批详情。
...
1. 启动①平台和③基础底座。
2. 调用生成的 `POST /api/entry-application/submit`。
3. 验证实例已创建、变量和银行回单已保存。
4. 验证 `apply` 历史任务已完成，下一用户任务已创建。
5. 使用③通用待办和审批动作继续经理审批、财务确认和办结。
